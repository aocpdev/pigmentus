...
const getCart = function (userId) {
    return pool.query(
        'SELECT c.id, c.user_id as "userId", c.quantity, c.product_id as "productId", u.name as "userName", u.email as "userEmail", p.name as "productName", p.customer_price as "customerPrice", p.weight, p.description, p.image FROM carts c JOIN products p ON p.id = c.product_id JOIN users u ON u.id = $1;',
        [
            userId
        ]);
}
